...
public class GenJsNativeJsr {
	 ...
	 * TODO
	 * 
	 * ..1. Arguments is not resolved to ArgumentsJsr.. ..2. Array can not be
	 * resolved to type .. ArrayJsr now ..3. Double methods length in RegExp...
	 * mixin issue ..4. Number should not be converted to NumberJsr ..5. Array
	 * was abstract now is not abstract during jsr gen only may be problem with
	 * Jst however STOPPED HERE ..6. TODO fix the Jsr imports for native types
	 * removing them only solves package issue ..7. TODO create jsr for browser
	 * types and vjo lib ..8. Hook up to prebuild task. Seperate projects for
	 * each one Vjo,JsNative, Browser ..9. Add these projects to Dsf library for
	 * now. ..10. Fix tests ... run more tests ..11. run build with new jsr
	 * generator
	 */
	...
	public static void main(String[] args) throws FileNotFoundException {
		...
		for (IJstType type : types) {
			...
			try {
				...
				// File jsFile = createFile(outputDir, null,
				// type.getSimpleName());
				// PrintWriter pw = new PrintWriter(jsFile);
				// VjoGenerator gen = new VjoGenerator(new
				// GeneratorCtx(CodeStyle.PRETTY));
				// gen.writeVjo(type);
				// String generatedText = gen.getGeneratedText();
				// pw.append(generatedText);
				// pw.flush();
				//
				// IScriptUnit isu = new VjoParser().parse("Js",
				// jsFile.getAbsolutePath(), generatedText, new TranslateCtx());
				//
				...
				if (type3.getPackage() == null) {
					File jsrFile = createJsrFile(outputDir, package1,
							type.getSimpleName());
	...
	private static File createJsrFile(String outputDir, JstPackage package1,
		...
		File dir = new File(outputDir + File.separatorChar + packageURI
				+ File.separatorChar);
		if (!dir.exists()) {
			dir.mkdirs();
		}
