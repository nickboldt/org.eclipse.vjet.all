...
public class BootstrapParser {
	...
	private static Map<? extends String, ? extends IJstType> addExtentionTypes(
		...
		Map<String, IJstType> mapOfTypes = new LinkedHashMap<String, IJstType>(
				extentionTypes.length);
	...
	// private static Map<String, IJstType> createCascade(List<JsBuilderDef>
	// defs) {
	//
	...
	private static Map<String, IJstType> processDefs(List<JsBuilderDef> defs,
			URL bootstrapFile) {
			...
			Map<String, List<JstMethod>> map = createJstMethodMap(def,
					commentList, new IFilter() {
						...
						public boolean filter(JsBuilderDef def, JstMethod m) {
							if (m.getRtnType().getName()
									.equals(def.getType().getName())) {
								return false;
							}
	...
	private static Map<String, ? extends JstType> processDef(JsBuilderDef def,
		...
		while (stItr.hasNext()) {
			...
			for (SectionInfo si : st.m_sections) {
				...
				if (methods != null) {
					addMethod(t, getType(si.m_nextState.m_id, baseName),
							methods);
				}
				// }
	...
	private static void calculateNextState(Map<String, StateInfo> states,
			JsBuilderDef def) {
	...
	private static void determineNextState(String state, SectionInfo info,
		...
		determineKey(currentState, info, states, lengthOfKey, candidateKeys,
				sections, def.getAnyOrderGroup(info.m_section.getMtd()));
					...
					while (gen.hasMore()) {
						...
						for (int j = 0; j < indices.length; j++) {
							...
							if (j == indices.length - 1) {
								...
								List<Section> samegrp = def
										.getAnyOrderGroup(section.getMtd());
								for (Section s : sections) {
									if (s.isAnyOrder()
											&& isInSameGrp(s, samegrp)) {
										continue;
									}
	...
	private static void printStates(Map<String, StateInfo> states) {
		...
		for (SectionInfo info : firstKey.m_sections) {
			...
			System.out.print(section.getMtd().substring(0, max > 4 ? 4 : max)
					+ "\t"); // KEEPME
	...
	private static JstType getType(final List<JsTypingMeta> list) {
		...
		for (JsTypingMeta m : list) {
			...
			if (typeFromCache == null && m instanceof JsAttributed) {
				...
				typeFromCache = JstCache.getInstance().getType(
						jsa.getAttributor().getType() + "." + jsa.getName());
			}
	...
	private static void handleOverloading(JstType t, JstType rtnType,
		...
		for (IJstMethod mtd : methods) {
			// TODO overloading
			if (first.getArgs().size() > 0
					&& first.getArgs().get(0).isOptional()) {
				argArr = new JstArg[0];
			}
	...
	private static JstMethod createNewJstMethod(IJstMethod first,
			JstArg[] argArr) {
		JstMethod dispatcher = new JstMethod(first.getName(),
				first.getModifiers(), first.getRtnType(), first.getArgs()
						.toArray(argArr));
		return dispatcher;
	}
	...
	public static Collection<IJsCommentMeta> collectComments(URL f)
		...
		AstCompilationResult astResult = SyntaxTreeFactory2
				.createASTCompilationResult(settings, charsource,
						f.toExternalForm(), encoding);
		CompilationUnitDeclaration compilationUnitDeclaration = astResult
				.getCompilationUnitDeclaration();
		JstSourceUtil jstSourceUtil = new JstSourceUtil(
	...
	private static void processMtds(JsBuilderDef def,
			Collection<IJsCommentMeta> list, Map<String, List<JstMethod>> map,
			IFilter filter) {
