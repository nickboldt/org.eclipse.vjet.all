...
/**
 * The model for the set of profiles which are available in 
he workbench.
 */
public abstract class ProfileManager extends Observable {
	...
	private final static String SHARED_PROFILE= "org.eclipse.vjet.ui.default.shared"; //$NON-NLS-1$
	...
	public ProfileManager(
		...
		if (context.getName() == ProjectScope.SCOPE && hasProjectSpecificSettings(context)) {
			...
			if (map != null) {
				...
				String projProfileId= context.getNode(VjetUIPlugin.PLUGIN_ID).get(fProfileKey, null);
	...
	protected String getSelectedProfileId(IScopeContext instanceScope) {
		String profileId= instanceScope.getNode(VjetUIPlugin.PLUGIN_ID).get(fProfileKey, null);
		if (profileId == null) {
			// request from bug 129427
			profileId= new DefaultScope().getNode(VjetUIPlugin.PLUGIN_ID).get(fProfileKey, null);
		}
	    return profileId;
    }
	...
	private Map readFromPreferenceStore(IScopeContext context, Profile workspaceProfile) {
		...
		IEclipsePreferences uiPrefs= context.getNode(VjetUIPlugin.PLUGIN_ID);
	...
	private void writeToPreferenceStore(Profile profile, IScopeContext context) {
		...
		final IEclipsePreferences uiPrefs= context.getNode(VjetUIPlugin.PLUGIN_ID);
	...
	protected void updateProfilesWithName(String oldName, Profile newProfile, boolean applySettings) {
		...
		for (int i= 0; i < projects.length; i++) {
			...
			IEclipsePreferences node= projectScope.getNode(VjetUIPlugin.PLUGIN_ID);
		...
		final IEclipsePreferences uiPrefs= instanceScope.getNode(VjetUIPlugin.PLUGIN_ID);
