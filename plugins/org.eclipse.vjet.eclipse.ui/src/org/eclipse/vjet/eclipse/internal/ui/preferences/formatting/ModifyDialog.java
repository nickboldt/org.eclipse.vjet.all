...
import org.eclipse.vjet.eclipse.internal.ui.preferences.formatting.ModifyDialogTabPage.IModificationListener;
import org.eclipse.vjet.eclipse.internal.ui.preferences.formatting.ProfileManager.CustomProfile;
import org.eclipse.vjet.eclipse.internal.ui.preferences.formatting.ProfileManager.Profile;
...
public abstract class ModifyDialog extends StatusDialog implements IModificationListener {
	...
	public ModifyDialog(Shell parentShell, Profile profile, ProfileManager profileManager, ProfileStore profileStore, boolean newProfile, String dialogPreferencesKey, String lastSavePathKey) {
		...
		fKeyPreferredWidth= VjetUIPlugin.PLUGIN_ID + dialogPreferencesKey + DS_KEY_PREFERRED_WIDTH;
		fKeyPreferredHight= VjetUIPlugin.PLUGIN_ID + dialogPreferencesKey + DS_KEY_PREFERRED_HEIGHT;
		fKeyPreferredX= VjetUIPlugin.PLUGIN_ID + dialogPreferencesKey + DS_KEY_PREFERRED_X;
		fKeyPreferredY= VjetUIPlugin.PLUGIN_ID + dialogPreferencesKey + DS_KEY_PREFERRED_Y;
		fKeyLastFocus= VjetUIPlugin.PLUGIN_ID + dialogPreferencesKey + DS_KEY_LAST_FOCUS;
		...
		fDialogSettings= VjetUIPlugin.getDefault().getDialogSettings();	
	}
	...
	private void saveButtonPressed() {
		...
		final String lastPath= VjetUIPlugin.getDefault().getDialogSettings().get(fLastSaveLoadPathKey + ".savepath"); //$NON-NLS-1$
		if (lastPath != null) {
		...
		VjetUIPlugin.getDefault().getDialogSettings().put(fLastSaveLoadPathKey + ".savepath", dialog.getFilterPath()); //$NON-NLS-1$
    ...
    private void doValidate() {
		...
		if (!name.equals(fProfile.getName()) && fProfileManager.containsName(name)) {
			updateStatus(new Status(IStatus.ERROR, VjetUIPlugin.PLUGIN_ID, FormatterMessages.ModifyDialog_Duplicate_Status));
			return;
		}
		
		if (fProfile.isBuiltInProfile() || fProfile.isSharedProfile()) {
			updateStatus(new Status(IStatus.INFO, VjetUIPlugin.PLUGIN_ID, FormatterMessages.ModifyDialog_NewCreated_Status));
			return;
		}
    ...
    private IStatus validateProfileName() {
	    ...
	    if (fProfile.isBuiltInProfile()) {
			if (fProfile.getName().equals(name)) { 
				return new Status(IStatus.ERROR, VjetUIPlugin.PLUGIN_ID, FormatterMessages.ModifyDialog_BuiltIn_Status);
			}	
    	}
    	
    	if (fProfile.isSharedProfile()) {
			if (fProfile.getName().equals(name)) { 
				return new Status(IStatus.ERROR, VjetUIPlugin.PLUGIN_ID, FormatterMessages.ModifyDialog_Shared_Status);
			}
    	}
		
		if (name.length() == 0) {
			return new Status(IStatus.ERROR, VjetUIPlugin.PLUGIN_ID, FormatterMessages.ModifyDialog_EmptyName_Status);
		}
