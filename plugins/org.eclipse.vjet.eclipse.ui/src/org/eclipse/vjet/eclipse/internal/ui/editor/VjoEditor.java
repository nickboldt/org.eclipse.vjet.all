...
public class VjoEditor extends JavaScriptEditor {
	...
	class JsSourceViewer extends AdaptedSourceViewer {

		public JsSourceViewer(Composite parent, IVerticalRuler verticalRuler,
				IOverviewRuler overviewRuler, boolean overviewRulerVisible,
				int styles, IPreferenceStore store) {
			super(parent, verticalRuler, overviewRuler, overviewRulerVisible,
					styles, store);
		}

		@Override
		public IFormattingContext createFormattingContext() {
			IFormattingContext context = new org.eclipse.vjet.eclipse.internal.ui.text.CommentFormattingContext();

			Map preferences;
			IModelElement inputJavaElement = getInputModelElement();
			IScriptProject javaProject = inputJavaElement != null ? inputJavaElement
					.getScriptProject() : null;
			if (javaProject == null)
				preferences = new HashMap(VjetPlugin.getOptions());
			else
				preferences = getFomatterSettings(javaProject);
		...
		private Map getFomatterSettings(IScriptProject project) {
			...
			PreferencesLookupDelegate context = new PreferencesLookupDelegate(
					project.getProject());

			String[] propertyNames = new String[defaultOptions.size()];

			propertyNames = (String[]) defaultOptions.keySet().toArray(
					propertyNames);

			Hashtable projectOptions = new Hashtable(propertyNames.length);
			for (int i = 0; i < propertyNames.length; i++) {
				String propertyName = propertyNames[i];
				String value = context.getString(VjetPlugin.PLUGIN_ID,
						propertyName);
				// if(value.equals("")){
				// // error case?
	...
	protected ISourceViewer createScriptSourceViewer(Composite parent,
		...
		return new JsSourceViewer(parent, verticalRuler, getOverviewRuler(),
	...
	public void createActions() {
		...
		RemoveOccurrenceAnnotations action = new RemoveOccurrenceAnnotations(
				this);
		action.setActionDefinitionId(IScriptEditorActionDefinitionIds.REMOVE_OCCURRENCE_ANNOTATIONS);
		...
		markAsSelectionDependentAction(DLTKActionConstants.ADD_BLOCK_COMMENT,
				true);
		bars.setGlobalActionHandler(
				"org.eclipse.vjet.eclipse.ui.AddJavaDocComment", addCommentAction); //$NON-NLS-1$
	...
	public void createPartControl(Composite parent) {
		...
		PlatformUI.getWorkbench().getHelpSystem()
				.setHelp(parent, IHelpContextIds.VJET_EDITOR);
	}
	...
	protected void doUpdateOccurrenceAnnotations(ITextSelection selection,
		...
		// modify by patrick
		VjoSelectionEngine vjoSelectionEngine = VjetUIUtils
				.getSelectionEngine();
	...
	private void enableVjoNature() {
		...
		boolean notShowDialog = pluginPreferences
				.getBoolean(VjoEditorMessages.VjoEditor_10);
		int result = pluginPreferences.getInt(VjoEditorMessages.VjoEditor_11);
		try {
			IEditorInput input = getEditorInput();
			if (input instanceof ExternalFileEditorInput) {// input is out of
															// work space
				return;
			} else if (input instanceof org.eclipse.dltk.mod.internal.debug.ui.ExternalFileEditorInput) {
				return;
			...
			if (!hasNature && !notShowDialog) {
				String message = MessageFormat.format(
						VjoEditorMessages.VjoEditor_13, new Object[] {
								this.getInputModelElement().getElementName(),
								project.getName() });
				EnableVjoNatureDialog md = new EnableVjoNatureDialog(Display
						.getCurrent().getActiveShell(),
						VjoEditorMessages.VjoEditor_12, getTitleImage(),
						message, SWT.ICON_INFORMATION, new String[] {
								VjoEditorMessages.VjoEditor_15,
								VjoEditorMessages.VjoEditor_16 }, 0);
				result = md.open();
				if (md.isSelection()) {
					pluginPreferences.setValue(VjoEditorMessages.VjoEditor_10,
							true);
					pluginPreferences.setValue(VjoEditorMessages.VjoEditor_11,
							result);
				}
			}

			IModelElement inputModelElement = getInputModelElement();
			if (getSourceViewerConfiguration() instanceof VjoSourceViewerConfiguration
					&& inputModelElement instanceof VjoSourceModule) {
	...
	protected void internalDoSetInput(IEditorInput input) throws CoreException {
		...
		if (original instanceof IVjoSourceModule
			...
			ITextFileBuffer buffer = manager.getTextFileBuffer(
					((ExternalStorageEditorInput) input).getStorage()
	...
	private void logError(Exception e) {
		VjetPlugin
				...
				.log(new Status(IStatus.ERROR, VjetPlugin.PLUGIN_ID,
						IStatus.ERROR, VjoEditorMessages.VjoEditor_21, e));
