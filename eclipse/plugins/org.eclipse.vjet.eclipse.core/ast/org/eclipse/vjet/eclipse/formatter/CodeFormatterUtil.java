...
public class CodeFormatterUtil {
	...
	public static int getTabWidth(IScriptProject project) {
		 ...
		 */
		String key;
		if (VjetPlugin.SPACE.equals(getCoreOption(project, DefaultCodeFormatterConstants.FORMATTER_TAB_CHAR)))
...
//		if (project == null)
//			return VjetPlugin.getOption(key);
...
//	}
	...
	public static String evaluateFormatterEdit(String string, TextEdit edit, Position[] positions) {
		try {
			...
			VjetPlugin.error("bug in the formatter",e); // bug in the formatter
	...
	public static TextEdit reformat(int kind, String string, int offset, int length, int indentationLevel, String lineSeparator, Map options) {
		...
		return JSFormatterFactory.createCodeFormatter(options, JSFormatterFactory.M_FORMAT_EXISTING)
	...
	public static TextEdit format2(ASTNode node, String str, int indentationLevel, String lineSeparator, Map options) {
		...
		if (node instanceof Statement) {
			...
			if (node.getASTType() == ASTNode.CASE_STATEMENT) {
				prefix= "switch(1) {"; //$NON-NLS-1$
				suffix= "}"; //$NON-NLS-1$
				code= CodeFormatter.K_STATEMENTS;
			}
		} else if (node.getASTType() == ASTNode.EXPRESSION && node.getASTType() != ASTNode.LOCAL_DECLARATION) { // vars
			code= CodeFormatter.K_EXPRESSION;
		} 
		else if (node.getASTType() == ASTNode.BLOCK ) {
