...
public abstract class ModifyDialogTabPage {
	...
	protected final class NumberPreference extends Preference {
		...
		private IStatus createErrorStatus() {
		    return new Status(IStatus.ERROR, VjetUIPlugin.PLUGIN_ID, 0, Messages.format(FormatterMessages.ModifyDialogTabPage_NumberPreference_error_invalid_value, new String [] {Integer.toString(fMinValue), Integer.toString(fMaxValue)}), null); 
		...
		protected void updateWidget() {
			...
			if (hasKey) {
			    ...
			    try {
			        ...
			        VjetUIPlugin.log(new Status(IStatus.ERROR, VjetPlugin.PLUGIN_ID, IStatus.OK, message, e));
			        s= ""; //$NON-NLS-1$
			    }
	...
	protected final static class DefaultFocusManager extends FocusAdapter {
		...
		private final static String PREF_LAST_FOCUS_INDEX= VjetUIPlugin.PLUGIN_ID + "formatter_page.modify_dialog_tab_page.last_focus_index"; //$NON-NLS-1$ 
