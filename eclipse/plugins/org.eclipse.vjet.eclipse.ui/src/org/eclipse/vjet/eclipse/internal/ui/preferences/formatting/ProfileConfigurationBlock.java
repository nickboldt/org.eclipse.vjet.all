...
public abstract class ProfileConfigurationBlock {
	...
	class ButtonController implements Observer, SelectionListener {
		...
		private void loadButtonPressed() {
			...
			final String lastPath= VjetUIPlugin.getDefault().getDialogSettings().get(fLastSaveLoadPathKey + ".loadpath"); //$NON-NLS-1$
			if (lastPath != null) {
			...
			if (path == null) 
				return;
			VjetUIPlugin.getDefault().getDialogSettings().put(fLastSaveLoadPathKey + ".loadpath", dialog.getFilterPath()); //$NON-NLS-1$
	...
	public ProfileConfigurationBlock(IProject project, final PreferencesAccess access, String lastSaveLoadPathKey) {
		...
		access.getInstanceScope().getNode(VjetUIPlugin.PLUGIN_ID).addPreferenceChangeListener(fPreferenceListener);
	...
	public Composite createContents(Composite parent) {
		...
		profileLabel.setText(VjetPreferenceMessages.CleanUpPreferencePage_Description);
	...
	public void performApply() {
		try {
			fCurrContext.getNode(VjetUIPlugin.PLUGIN_ID).flush();
			fCurrContext.getNode(VjetPlugin.PLUGIN_ID).flush();
			if (fCurrContext != fInstanceScope) {
				fInstanceScope.getNode(VjetUIPlugin.PLUGIN_ID).flush();
				fInstanceScope.getNode(VjetPlugin.PLUGIN_ID).flush();
			}
	...
	public void dispose() {
		if (fPreferenceListener != null) {
			fPreferenceAccess.getInstanceScope().getNode(VjetUIPlugin.PLUGIN_ID).removePreferenceChangeListener(fPreferenceListener);
