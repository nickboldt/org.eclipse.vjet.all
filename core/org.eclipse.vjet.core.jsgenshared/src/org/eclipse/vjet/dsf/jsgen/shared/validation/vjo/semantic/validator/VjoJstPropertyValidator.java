...
public class VjoJstPropertyValidator extends VjoSemanticValidator implements
	...
	public void onPreAllChildrenEvent(final IVjoValidationVisitorEvent event) {
		...
		if (jstProperty.getName() != null) {
			...
			final InvalidIdentifierNameWithKeywordRuleCtx idRuleCtx = new InvalidIdentifierNameWithKeywordRuleCtx(
					jstProperty.getName(), ctx.getGroupId(), new String[] {
							jstPropertyName, jstPropertyName },
					jstPropertyName, jstProperty.getOwnerType() != null
							&& jstProperty.getOwnerType().isEnum());
			satisfyRule(
					ctx,
					VjoSemanticRuleRepo.getInstance().INVALID_IDENTIFIER_WITH_KEYWORD,
					idRuleCtx);
		}

		// bugfix 6759 & 6760
		// method couldn't be both abstract and static or both abstract and
		// private
		if (jstProperty.getModifiers().isAbstract()) {
			if (jstProperty.getModifiers().isStatic()) {
				final BaseVjoSemanticRuleCtx ruleCtx = new BaseVjoSemanticRuleCtx(
						jstProperty.getName(), ctx.getGroupId(),
						new String[] { jstProperty.getName().getName() });
				satisfyRule(
						ctx,
						VjoSemanticRuleRepo.getInstance().METHOD_OR_PROPERTY_SHOULD_NOT_BE_BOTH_STATIC_AND_ABSTRACT,
						ruleCtx);
			}
			if (jstProperty.getModifiers().isPrivate()) {
				...
				satisfyRule(
						ctx,
						VjoSemanticRuleRepo.getInstance().METHOD_OR_PROPERTY_SHOULD_NOT_BE_BOTH_PRIVATE_AND_ABSTRACT,
						ruleCtx);
			}
		}
		...
		if (!jstProperty.isStatic() && ownerType.isClass()
				&& jstProperty.getName() != null) {// only ctype has inheritance
													// which could hide super
													// field
				...
				while (extType != null) {
					if (extType.getProperty(propertyName, false, false) != null) {
						...
						final BaseVjoSemanticRuleCtx ruleCtx = new BaseVjoSemanticRuleCtx(
								jstProperty.getName(),
								ctx.getGroupId(),
								new String[] { propertyName, extType.getName() });
						satisfyRule(
								ctx,
								VjoSemanticRuleRepo.getInstance().PROPERTY_SHOULD_NOT_HIDE_PARENT_PROPERTY,
								ruleCtx);
						break;
					}
	...
	public void onPostAllChildrenEvent(final IVjoValidationVisitorEvent event) {
		...
		// bugfix 5297, objLiteral is a value instead of an initializer expr
			...
			if (jstProperty.getValue() instanceof IExpr) {
				validatePropertyExpr(ctx, jstProperty,
						(IExpr) jstProperty.getValue());
			}
		}
	}
	...
	private void validatePropertyExpr(final VjoValidationCtx ctx,
		...
		if (initExprValue != null) {
			...
			if (!"Object".equals(initExprValue.getName())
					|| !(propertyExpr instanceof SimpleLiteral)) {// bugfix new
																	// Object()
																	// case;
																	// 8311
			...
			// enum properties are not actual jst properties, skip its
			// initialization validation
			if (jstProperty.getOwnerType() != null
					&& jstProperty
							.getOwnerType()
							.getAllPossibleProperties(jstProperty.isStatic(),
									true).contains(jstProperty)) {
				validatePropertyExprInitialization(ctx, jstProperty,
						propertyExpr);
			}

			if (jstProperty.getType() != null
					&& !TypeCheckUtil.isAssignable(jstProperty.getType(),
							initExprValue)) {
				...
				final BaseVjoSemanticRuleCtx ruleCtx = new BaseVjoSemanticRuleCtx(
						propertyExpr, ctx.getGroupId(), arguments);
				satisfyRule(ctx, VjoSemanticRuleRepo.getInstance().ASSIGNABLE,
						ruleCtx);
	...
	/**
	 * <p>
	 * this validation method aims to report initialization issues which
	 * property initialization value depends on uninitialized type this
	 * validation logic used to be separated into FieldAccessExpr,
	 * MtdInvocationExpr validators, now it's moved into Property validator
	 * 
	 * the output is initialization successfulness, it fails on the 1st left
	 * most child expr which has an uninitialized type in use the visit order is
	 * depth-first, visits stops on 1st error
	 * </p>
	 * 
	 * @param ctx
	 * @param property
	 ...
	 * @return
	 */
	private boolean validatePropertyExprInitialization(
			final VjoValidationCtx ctx, final IJstProperty property,
		...
		for (IJstNode child : propertyExpr.getChildren()) {
			if (child != null && child instanceof IExpr
					&& !(child instanceof BaseJsCommentMetaNode<?>)) {// only
																		// IExpr
																		// matters
		...
		if ((!isNative(propertyExprResultType) && !isVj$Type(propertyExprResultType) && !isDefiningTypeRef(propertyExprResultType,ctx.getJstTypeNames())) ) {
			...
			final BaseVjoSemanticRuleCtx ruleCtx = new BaseVjoSemanticRuleCtx(
					...
					new String[] {
							...
							propertyExprResultType != null ? propertyExprResultType
									.getName() : property.getInitializer()
									.toExprText() });
			satisfyRule(
					ctx,
					VjoSemanticRuleRepo.getInstance().CANNOT_USE_UNINTIALIZED_TYPE,
					ruleCtx);
			return false;
		}
	...
	private boolean isNative(IJstType propertyExprResultType) {
		...
		if (propertyExprResultType instanceof JstArrayInitializer
				|| propertyExprResultType instanceof JstArray) {
			return true;
		}
		if (propertyExprResultType == null
				|| propertyExprResultType.getPackage() == null) {
			return false;
		}
	...
	protected void validatePropertyType(final VjoValidationCtx ctx,
		...
		validateComplexType(ctx, jstProperty, jstProperty.getName().getName(),
				propertyJstType);
	}
}
