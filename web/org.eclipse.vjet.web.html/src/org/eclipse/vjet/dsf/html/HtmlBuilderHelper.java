...
//	}
	...
	{		
		...
		try {			
			if (schema != null) {
				// custom Html schema
				parser.setProperty(Parser.schemaProperty, schema); // Set
			}
			...
			parser.setFeature(Parser.defaultAttributesFeature, 
					(buildOption==null || !buildOption.isNoDefaultAttr())?true:false);	

			// DISABLE parser wellformed tags with upgrade to tagsoup 1.2
