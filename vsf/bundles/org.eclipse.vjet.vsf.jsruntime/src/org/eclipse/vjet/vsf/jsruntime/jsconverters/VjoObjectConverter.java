...
import org.eclipse.vjet.dsf.serializer.VjoSerializer;
import org.eclipse.vjet.dsf.serializer.VjoSerializer.VjoSerializerOption;

public class VjoObjectConverter extends BaseConverter<String> {

	private static final List<Class> VALID_CONVERSION_TYPES;
	static {
		final Class[] types = { ISerializableForVjo.class };

		VALID_CONVERSION_TYPES = immutableTypeList(types);
	}

	public IConversionResult<String> convert(Object toBeConverted) {
		ISerializableForVjo content = (ISerializableForVjo) toBeConverted;
		return setResult(toBeConverted, VjoSerializer.getInstance().toVjo(content, VjoSerializerOption.DATA_REGISTRY_FREE));
	}
